...
#   include <boost/pool/detail/pool_construct_simple.ipp>
    ...
    void clone_to(object_pool<T, UserAllocator> & clone) const {
        pool<UserAllocator>::clone_to(clone);
    }
    // added by arch.jslin 2012.11
    void restore(object_pool<T, UserAllocator> & backup) {
        pool<UserAllocator>::restore(backup);
    }
};
...
object_pool<T, UserAllocator>::~object_pool()
{
  ...
  do
  {
    ...
    if( !this->i_am_clone_ ) {
        // 2012.11 arch.jslin: if this pool is a clone, then the free list would be totally of no use when destructing,
        //                     since it points to the old memory chunks.
