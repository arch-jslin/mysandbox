...
*/
  ...
  private:
    ...
    {
      return (ptr_id() - sizeof(size_type));
    }
  ...
  public:
    ...
    size_type & id() const
    { //! designated by the time order of PODptr's creation.
      //! It basically has no meaning, but it can identify which PODptr is which.
      return *(static_cast<size_type *>(static_cast<void*>((ptr_id()))));
    }
    size_type element_size() const
    { //! \returns size of element pointer area.
      return static_cast<size_type>(sz - (sizeof(size_type) * 2) -
          math::static_lcm<sizeof(size_type), sizeof(void *)>::value);
    }
...
class pool: protected simple_segregated_storage < typename UserAllocator::size_type >
{
  ...
  protected:
    ...
    size_type max_size;
    bool i_am_clone_;
    size_type podptr_count_; //! Just a counter counting how many unique PODptrs have been created.
  ...
  public:
    ...
    :
        ...
        max_size(nmax_size), i_am_clone_(false), podptr_count_(0)
...
//        }
        ...
        clone.max_size      = max_size;
        clone.i_am_clone_   = true;
        clone.podptr_count_ = podptr_count_;
...
//        }
        ...
        do {
            details::PODptr<size_type> cloned_ptr((UserAllocator::malloc)(iter.total_size()), iter.total_size());
            memcpy(cloned_ptr.begin(), iter.begin(), iter.total_size());
            if( last_cloned_ptr.valid() ) {
                last_cloned_ptr.next(cloned_ptr);
            } else { // this will only happen once when you are cloning first block
                clone.list = cloned_ptr;
            }
...
//        }
        ...
        //start_size, requested_size and max_size should never change

        details::PODptr<size_type> iter = backup.list; // start from head and copy it

        if( !iter.valid() ) return; // if it's very sure nothing to copy

        details::PODptr<size_type> last_restored_ptr;
        details::PODptr<size_type> restoring_ptr = list;
        details::PODptr<size_type> restoring_ptr_next;
        details::PODptr<size_type> iter_next;
        ...
        do {
            ...
            restoring_ptr_next = restoring_ptr.next();
            iter_next = iter.next();
            memcpy(restoring_ptr.begin(), iter.begin(), iter.total_size());
