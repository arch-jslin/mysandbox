...
-- This script demonstrate how to use LuaJIT2 FFI to integrate some simple 
-- SDL & GL function out-of-the-box. (well, almost.) Make sure your computer 
-- supports OpenGL VBO & PBO, this code is for demonstration purpose only, 
-- didn't do any check on this.
...
-- Script usage:
-- luajit lifegame_ffi_sdl_gl.lua <cell_size> <width> <height> [no_model_jit]
-- e.g luajit lifegame_ffi_sdl_gl.lua 4 256 256 no_model_jit
...
function game:init()
  ...
  self.csize        = tonumber(arg[1]) or 2
  self.model_w      = tonumber(arg[2]) or 120 
  self.model_h      = tonumber(arg[3]) or 90 
  self.NO_MODEL_JIT = arg[4] == 'no_model_jit' and true or false  
  ...
  self:createTexture()  --create Texture for render usage (search for render_)
...
local function main()                    -- main is called at the end of script
  ...
  while game:run(event) do
    game:render()                        -- draw using PBO only for debug purpose 
